...
name = "";
var commands = {
    "help": "<br>help - To get help commands<br>date - To get current date<br>qual - To get my qualification details<br>about - About me!<br>skills - Check out my Skillzz<br>contact - Hey, How you doing?<br>fb - Stalk me in fb!<br>linkedin - Check out me as a professional<br>github - Care to look?<br>resume - Need to look!<br><br>",
...
function addline(state) {
    ...
    if (register == 0) {
        ...
        if (detail == 5) {
            ...
            $(".butt").attr("disabled", "disabled");
            if (c < 4) height += Number(document.getElementById("entry" + String(c - 1)).offsetHeight);
            $('#form').css({
        ...
        $(".butt").attr("disabled", "disabled");
        $("span").removeClass('blink');
        if (regnum == 3) $("#answer" + String(regnum)).append(entry);
        else
...
function check_string(c, state) {
    if (register == 0) {
        substr = '';
        text = '';
        if (state == 0) text = $("#gitname").val().toLowerCase()
        else
            text = $("#text" + String(c)).val().toLowerCase();
        if (commands.hasOwnProperty(text)) substr = commands[text];
        else
        ...
        if (text != "register") {
            ...
            $("span").removeClass('blink');
            if (state == 0) $("#answer" + String(c)).append(entry);
            else
        ...
        if (text == "linkedin") {
      	...
      	if (text == "resume") {
            window.open("assets/Ganesh_Raghavendran_Resume_2018.pdf");
        }
        if (text == "sponsor") {
            $("#sponblock").show();
            $("#sponsors").show();
...
function increase() {
    ...
    if (count == 1 && document.getElementById("name1").value == "") err = 1;
    if (err == 0) {
        count++;
        $(".butt").attr("disabled", "disabled");
        if (count == 3) {
